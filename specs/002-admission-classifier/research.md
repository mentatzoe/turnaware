...
## Decision: Make the product classifier path primary; keep deterministic as explicit evidence path

**Rationale**: TUR-11 is meant to complete the actual admission classifier, not promote the offline deterministic harness into the product path. The constitution still requires CI-verifiable verdict behavior without live stochastic providers, so deterministic evidence remains mandatory, but it is selected explicitly for tests/offline review and must not masquerade as the default or only implemented classifier.
...
- Provider-backed/product classifier as the only path: rejected because CI/offline verification would depend on network/credentials and could not be the mandatory done bar.
...
**Rationale**: Hosts must know which classifier path/configuration produced a verdict. A machine-readable result field keeps CLI and callable core equivalent and is easier to test than prose-only audit logs.
...
**Alternatives considered**:
- Fallback to deterministic default: rejected because it contradicts FR-007 and makes host configuration non-auditable.
...
**Rationale**: The current fixed `0.85/0.05` distribution masks conflict and makes keyword-shaped matches look equally certain. Both the product classifier and deterministic evidence path should lower confidence and surface reasons when context signals conflict or missing evidence.
...
- Add broad benchmark corpus: rejected for this slice; deterministic adversarial fixtures are the required evidence.
